...
//		System.out.println(boardList);
	...
	// 게시판 목록 - 페이징
	public Map<String, Object> boardListPaging(int crtPage) {
		...
		List<BoardVo> boardList = boardDao.boardListPaging(startRNum, endRNum);
		 ...
		 * listCnt * endPageBtnNo < totalCnt	-> true
		 */
		boolean next = false;
		if(listCnt * endPageBtnNo < totalCnt) {
			next = true;
		} else {	// 다음버튼이 없을 때(false) - endPageBtnNo 다시 계산
			/*
			 * (int)Math.ceil(전체글갯수 / (double)마지막버튼번호)
			 * 157/10	->	157/10.0	->	15.7	-> 16.0	->	16
			 */
